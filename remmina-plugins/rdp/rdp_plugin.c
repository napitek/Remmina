...
static BOOL rf_process_event_queue(RemminaProtocolWidget * gp)
{
	...
	while ((event = (RemminaPluginRdpEvent *) g_async_queue_try_pop(rfi->event_queue)) != NULL) {
		...
		case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_DATA_RESPONSE:
			rfi->clipboard.context->ClientFormatDataResponse(rfi->clipboard.context,
									 event->clipboard_formatdataresponse.
									 pFormatDataResponse);
		...
		case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_DATA_REQUEST:
			rfi->clipboard.context->ClientFormatDataRequest(rfi->clipboard.context,
									event->clipboard_formatdatarequest.
									pFormatDataRequest);
...
BOOL rf_auto_reconnect(rfContext * rfi)
{
	...
	remmina_plugin_service->
	    log_printf("[RDP][%s] network disconnection detected, initiating reconnection attempt\n",
	...
	while (TRUE) {
		...
		remmina_plugin_service->log_printf("[RDP][%s] attempting reconnection, attempt #%d of %d\n",
						   rfi->settings->ServerHostname, rfi->reconnect_nattempt,
						    ...
						    const char *old_issuer, const char *old_fingerprint)
{
	...
	status =
	    remmina_plugin_service->protocol_plugin_changed_certificate(gp, subject, issuer, new_fingerprint,
									old_fingerprint);
...
static int remmina_rdp_receive_channel_data(freerdp * instance, int channelId, UINT8 * data, int size, int flags,
					    int total_size)
{
...
static gboolean remmina_rdp_main(RemminaProtocolWidget * gp)
{
	...
	rfi->settings->AutoReconnectionEnabled =
	...
	if (remmina_plugin_service->file_get_string(remminafile, "gateway_domain")) {
		rfi->settings->GatewayDomain =
		    strdup(remmina_plugin_service->file_get_string(remminafile, "gateway_domain"));
	...
	if (remmina_plugin_service->file_get_string(remminafile, "gateway_username")) {
		rfi->settings->GatewayUsername =
		    strdup(remmina_plugin_service->file_get_string(remminafile, "gateway_username"));
		rfi->settings->GatewayUseSameCredentials = FALSE;
	}
	...
	if (rfi->settings->GatewayEnabled)
		freerdp_set_gateway_usage_method(rfi->settings,
						 remmina_plugin_service->file_get_int(remminafile, "gateway_usage",
										      FALSE) ? TSC_PROXY_MODE_DETECT :
	...
	if (remmina_plugin_service->file_get_string(remminafile, "loadbalanceinfo")) {
		rfi->settings->LoadBalanceInfo =
		    (BYTE *) strdup(remmina_plugin_service->file_get_string(remminafile, "loadbalanceinfo"));
	...
	if (remmina_plugin_service->file_get_string(remminafile, "execpath")) {
		rfi->settings->ShellWorkingDirectory =
		    strdup(remmina_plugin_service->file_get_string(remminafile, "execpath"));
	}
			...
			if (rdpsnd_rate > 1000 && rdpsnd_rate < 150000) {
				...
				if (cs) {
					...
					if (rdpsnd_channel >= 1 && rdpsnd_channel <= 2) {
						snprintf(rdpsnd_param2, sizeof(rdpsnd_param2), "channel:%d",
							 rdpsnd_channel);
	...
	rfi->settings->RedirectClipboard =
	    (remmina_plugin_service->file_get_int(remminafile, "disableclipboard", FALSE) ? FALSE : TRUE);
	...
	if (!freerdp_connect(rfi->instance)) {
		if (!rfi->user_cancelled) {
			...
			case FREERDP_ERROR_CONNECT_FAILED:
				remmina_plugin_service->protocol_plugin_set_error(gp,
										  _
										  ("Connection to RDP server %s failed."),
										  rfi->settings->ServerHostname);
				break;
			case FREERDP_ERROR_DNS_NAME_NOT_FOUND:
				remmina_plugin_service->protocol_plugin_set_error(gp,
										  _
										  ("Unable to find the address of RDP server %s."),
										  rfi->settings->ServerHostname);
				break;
			default:
				remmina_plugin_service->protocol_plugin_set_error(gp,
...
static gpointer remmina_rdp_main_thread(gpointer data)
{
	...
	CANCEL_ASYNC gp = (RemminaProtocolWidget *) data;
...
static gboolean remmina_rdp_close_connection(RemminaProtocolWidget * gp)
{
	...
	if (freerdp_get_last_error(rfi->instance->context) == 0x10005) {
		remmina_plugin_service->protocol_plugin_set_error(gp,
								  "Another user connected to the server (%s), forcing the disconnection of the current connection.",
								  rfi->settings->ServerHostname);
	}
...
static const RemminaProtocolSetting remmina_rdp_basic_settings[] = {
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disableautoreconnect", N_("Disable automatic reconnection"), FALSE,
	 NULL, NULL},
...
static const RemminaProtocolSetting remmina_rdp_advanced_settings[] = {
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disablepasswordstoring", N_("Disable password storing"), TRUE, NULL,
	 NULL},
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "gateway_usage", N_("Server detection using RD Gateway"), FALSE, NULL,
	 NULL},
	{REMMINA_PROTOCOL_SETTING_TYPE_END, NULL, NULL, FALSE, NULL, NULL}
};
...
static const RemminaProtocolFeature remmina_rdp_features[] = {
	...
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_RDP_FEATURE_TOOL_SENDCTRLALTDEL, N_("Send Ctrl+Alt+Delete"), NULL,
	 NULL},
...
G_MODULE_EXPORT gboolean remmina_plugin_entry(RemminaPluginService * service)
{
	...
	if (vermaj < FREERDP_REQUIRED_MAJOR ||
		...
		g_printf("Unable to load RDP plugin due to bad freerdp library version. Required "
			 ...
			 FREERDP_REQUIRED_MAJOR, FREERDP_REQUIRED_MINOR, FREERDP_REQUIRED_REVISION, vermaj, vermin,
			 verrev);
		return FALSE;
	}
