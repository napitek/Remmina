...
static gboolean remmina_rdp_event_on_draw(GtkWidget * widget, cairo_t * context, RemminaProtocolWidget * gp)
{
	...
	if (rfi->is_reconnecting) {
		...
		cairo_move_to(context, (width - (extents.width + extents.x_bearing)) / 2,
...
static gboolean remmina_rdp_event_on_configure(GtkWidget * widget, GdkEventConfigure * event,
...
static void remmina_rdp_event_reverse_translate_pos_reverse(RemminaProtocolWidget * gp, int ix, int iy, int *ox,
							    int *oy)
{
		...
		if (!rfi->use_client_keymap) {
			...
			cooked_keycode =
			    XKeysymToKeycode(GDK_DISPLAY_XDISPLAY(display),
					     XkbKeycodeToKeysym(GDK_DISPLAY_XDISPLAY(display), event->hardware_keycode,
								0, 0));
...
void remmina_rdp_event_init(RemminaProtocolWidget * gp)
{
	...
	g_signal_connect(G_OBJECT(rfi->drawing_area), "focus-in-event", G_CALLBACK(remmina_rdp_event_on_focus_in), gp);
...
void remmina_rdp_event_uninit(RemminaProtocolWidget * gp)
{
	...
	if (rfi->clipboard.clipboard_handler) {
		g_signal_handler_disconnect(G_OBJECT
					    (gtk_widget_get_clipboard(rfi->drawing_area, GDK_SELECTION_CLIPBOARD)),
					    rfi->clipboard.clipboard_handler);
...
static void remmina_rdp_event_create_cairo_surface(rfContext * rfi)
{
	...
	rfi->surface =
	    cairo_image_surface_create_for_data((unsigned char *) rfi->primary_buffer, rfi->cairo_format, rfi->width,
						rfi->height, stride);
}
...
static BOOL remmina_rdp_event_create_cursor(RemminaProtocolWidget * gp, RemminaPluginRdpUiObject * ui)
{
	...
	if (freerdp_image_copy_from_pointer_data((BYTE *) data, PIXEL_FORMAT_BGRA32,
	...
	surface =
	    cairo_image_surface_create_for_data(data, CAIRO_FORMAT_ARGB32, pointer->width, pointer->height,
