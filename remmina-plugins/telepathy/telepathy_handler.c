...
static void remmina_tp_handler_handle_channels(TpSvcClientHandler * handler, const char *account_path,
					       const char *connection_path, const GPtrArray * channels,
					       const GPtrArray * requests_satisfied, guint64 user_action_time,
					       GHashTable * handler_info, DBusGMethodInvocation * context)
{
...
static gboolean remmina_tp_handler_register(RemminaTpHandler * handler)
{
	...
	dbus_g_connection_register_G_OBJECT(tp_proxy_get_dbus_connection(TP_PROXY(bus)),
