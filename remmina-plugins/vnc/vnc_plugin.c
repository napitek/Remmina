...
static void remmina_plugin_vnc_event_push(RemminaProtocolWidget * gp, gint event_type, gpointer p1, gpointer p2,
					  gpointer p3)
{
...
gboolean remmina_plugin_vnc_setcursor(RemminaProtocolWidget * gp)
{
	...
	if (gpdata->queuecursor_surface) {
		cur =
		    gdk_cursor_new_from_surface(gdk_display_get_default(), gpdata->queuecursor_surface,
						gpdata->queuecursor_x, gpdata->queuecursor_y);
...
static void remmina_plugin_vnc_rfb_updatefb(rfbClient * cl, int x, int y, int w, int h)
{
	...
	if (w >= 1 || h >= 1) {
		...
		remmina_plugin_vnc_rfb_fill_buffer(cl,
						   cairo_image_surface_get_data(gpdata->rgb_buffer) + y * rowstride +
						   x * 4, rowstride,
						   gpdata->vnc_buffer + ((y * width + x) * bytesPerPixel),
						   width * bytesPerPixel, NULL, w, h);
...
static gboolean remmina_plugin_vnc_queue_cuttext(RemminaPluginVncCuttextParam * param)
{
	...
	if (GTK_IS_WIDGET(gp) && gpdata->connected) {
		...
		if (diff >= 10) {
			...
			gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_CLIPBOARD), param->text,
					       param->textlen);
		}
	}
	g_free(param->text);
...
static char *remmina_plugin_vnc_rfb_password(rfbClient * cl)
{
	...
	if (!pwd) {
		...
		ret =
		    remmina_plugin_service->protocol_plugin_init_authpwd(gp, REMMINA_AUTHPWD_TYPE_PROTOCOL,
									 !disablepasswordstoring);
...
static rfbCredential *remmina_plugin_vnc_rfb_credential(rfbClient * cl, int credentialType)
{
	...
	case rfbCredentialTypeUser:
		...
		if (gpdata->auth_first && s1 && s2) {
			...
			disablepasswordstoring =
			    remmina_plugin_service->file_get_int(remminafile, "disablepasswordstoring", FALSE);
			ret =
			    remmina_plugin_service->protocol_plugin_init_authuserpwd(gp, FALSE,
										     !disablepasswordstoring);
	...
	case rfbCredentialTypeX509:
		if (gpdata->auth_first && remmina_plugin_service->file_get_string(remminafile, "cacert")) {
			...
			if (ret == GTK_RESPONSE_OK) {
				cred->x509Credential.x509CACertFile =
				    remmina_plugin_service->protocol_plugin_init_get_cacert(gp);
				cred->x509Credential.x509CACrlFile =
				    remmina_plugin_service->protocol_plugin_init_get_cacrl(gp);
				cred->x509Credential.x509ClientCertFile =
...
static void remmina_plugin_vnc_rfb_cursor_shape(rfbClient * cl, int xhot, int yhot, int width, int height,
						int bytesPerPixel)
{
	...
	if (g_strcmp0(f, "VNC connection failed: %s") == 0) {
		...
		if (vnc_encryption_disable_requested) {
			ff = g_strconcat(_("Unknown authentication scheme from VNC server: %s"),
					 ". ", _("Please retry after enabling encryption on this profile."), NULL);
...
static gboolean remmina_plugin_vnc_main(RemminaProtocolWidget * gp)
{
	...
	while (gpdata->connected) {
		...
		if (remmina_plugin_service->file_get_string(remminafile, "proxy")) {
			...
			remmina_plugin_service->get_server_port(remmina_plugin_service->
								file_get_string(remminafile, "proxy"), 5900, &s,
								&cl->serverPort);
			cl->serverHost = strdup(s);
			g_free(s);
		}
		...
		remmina_plugin_vnc_update_colordepth(cl,
						     remmina_plugin_service->file_get_int(remminafile, "colordepth",
											  15));
...
static gpointer remmina_plugin_vnc_main_thread(gpointer data)
{
	...
	CANCEL_ASYNC remmina_plugin_vnc_main((RemminaProtocolWidget *) data);
	return NULL;
}


static RemminaPluginVncCoordinates remmina_plugin_vnc_scale_coordinates(GtkWidget * widget, RemminaProtocolWidget * gp,
									gint x, gint y)
{
	GtkAllocation widget_allocation;
...
static gboolean remmina_plugin_vnc_on_motion(GtkWidget * widget, GdkEventMotion * event, RemminaProtocolWidget * gp)
{
	...
	remmina_plugin_vnc_event_push(gp, REMMINA_PLUGIN_VNC_EVENT_POINTER, GINT_TO_POINTER(coordinates.x),
				      GINT_TO_POINTER(coordinates.y), GINT_TO_POINTER(gpdata->button_mask));
	return TRUE;
}
...
static gboolean remmina_plugin_vnc_on_scroll(GtkWidget * widget, GdkEventScroll * event, RemminaProtocolWidget * gp)
{
	...
	coordinates = remmina_plugin_vnc_scale_coordinates(widget, gp, event->x, event->y);
	remmina_plugin_vnc_event_push(gp, REMMINA_PLUGIN_VNC_EVENT_POINTER, GINT_TO_POINTER(coordinates.x),
				      GINT_TO_POINTER(coordinates.y), GINT_TO_POINTER(mask | gpdata->button_mask));
	remmina_plugin_vnc_event_push(gp, REMMINA_PLUGIN_VNC_EVENT_POINTER, GINT_TO_POINTER(coordinates.x),
				      GINT_TO_POINTER(coordinates.y), GINT_TO_POINTER(gpdata->button_mask));
...
static gboolean remmina_plugin_vnc_on_key(GtkWidget * widget, GdkEventKey * event, RemminaProtocolWidget * gp)
{
	...
	keyval =
	    remmina_plugin_service->pref_keymap_get_keyval(remmina_plugin_service->
							   file_get_string(remminafile, "keymap"), event_keyval);
...
static void remmina_plugin_vnc_on_cuttext_request(GtkClipboard * clipboard, const gchar * text,
						  RemminaProtocolWidget * gp)
{
...
static void remmina_plugin_vnc_on_cuttext(GtkClipboard * clipboard, GdkEvent * event, RemminaProtocolWidget * gp)
{
	...
	gtk_clipboard_request_text(clipboard, (GtkClipboardTextReceivedFunc) remmina_plugin_vnc_on_cuttext_request,
				   gp);
}
...
static void remmina_plugin_vnc_on_realize(RemminaProtocolWidget * gp, gpointer data)
{
	...
	if (remmina_plugin_service->file_get_int(remminafile, "showcursor", FALSE)) {
		...
		cursor =
		    gdk_cursor_new_from_pixbuf(gdk_display_get_default(), pixbuf, dot_cursor_x_hot, dot_cursor_y_hot);
...
static gboolean remmina_plugin_vnc_open_connection(RemminaProtocolWidget * gp)
{
	...
	g_signal_connect(G_OBJECT(gpdata->drawing_area), "key-release-event", G_CALLBACK(remmina_plugin_vnc_on_key),
			 gp);

	if (!remmina_plugin_service->file_get_int(remminafile, "disableclipboard", FALSE)) {
		gpdata->clipboard_handler = g_signal_connect(G_OBJECT(gtk_clipboard_get(GDK_SELECTION_CLIPBOARD)),
							     "owner-change", G_CALLBACK(remmina_plugin_vnc_on_cuttext),
							     gp);
	}
...
static void remmina_plugin_vnc_call_feature(RemminaProtocolWidget * gp, const RemminaProtocolFeature * feature)
{
	...
	case REMMINA_PLUGIN_VNC_FEATURE_PREF_DISABLESERVERINPUT:
		PermitServerInput((rfbClient *) (gpdata->client),
				  remmina_plugin_service->file_get_int(remminafile, "disableserverinput",
								       FALSE) ? 1 : 0);
		break;
...
static gboolean remmina_plugin_vnc_on_draw(GtkWidget * widget, cairo_t * context, RemminaProtocolWidget * gp)
{
	...
	if (remmina_plugin_service->protocol_plugin_get_scale(gp)) {
		gtk_widget_get_allocation(widget, &widget_allocation);
		cairo_scale(context,
			    (double) widget_allocation.width / width, (double) widget_allocation.height / height);
	}
...
static const RemminaProtocolSetting remmina_plugin_vnc_advanced_settings[] = {
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disablepasswordstoring", N_("Disable password storing"), FALSE, NULL,
	 NULL},
	{REMMINA_PROTOCOL_SETTING_TYPE_END, NULL, NULL, FALSE, NULL, NULL}
};
...
static const RemminaProtocolFeature remmina_plugin_vnc_features[] = {
	...
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PLUGIN_VNC_FEATURE_TOOL_CHAT, N_("Open Chat..."), "face-smile",
	 NULL},
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PLUGIN_VNC_FEATURE_TOOL_SENDCTRLALTDEL,
