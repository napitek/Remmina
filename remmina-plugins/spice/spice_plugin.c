...
static void remmina_plugin_spice_init(RemminaProtocolWidget * gp)
{
	...
	g_object_set(gpdata->gtk_session,
		     "auto-clipboard",
		     !remmina_plugin_service->file_get_int(remminafile, "disableclipboard", FALSE), NULL);
}
...
static gboolean remmina_plugin_spice_close_connection(RemminaProtocolWidget * gp)
{
	...
	if (gpdata->main_channel) {
		g_signal_handlers_disconnect_by_func(gpdata->main_channel,
						     G_CALLBACK(remmina_plugin_spice_main_channel_event_cb), gp);
	}
...
static void remmina_plugin_spice_channel_new_cb(SpiceSession * session, SpiceChannel * channel,
						RemminaProtocolWidget * gp)
{
	...
	if (SPICE_IS_MAIN_CHANNEL(channel)) {
		...
		g_signal_connect(channel,
				 "new-file-transfer", G_CALLBACK(remmina_plugin_spice_file_transfer_new_cb), gp);
#endif				/* SPICE_GTK_CHECK_VERSION(0, 31, 0) */
#endif				/* SPICE_GTK_CHECK_VERSION */
	}
	...
	if (SPICE_IS_DISPLAY_CHANNEL(channel)) {
		...
		g_signal_connect(gpdata->display,
				 "notify::ready", G_CALLBACK(remmina_plugin_spice_display_ready_cb), gp);
...
static gboolean remmina_plugin_spice_ask_auth(RemminaProtocolWidget * gp)
{
	...
	if (ret == GTK_RESPONSE_OK) {
		g_object_set(gpdata->session,
			     "password", remmina_plugin_service->protocol_plugin_init_get_password(gp), NULL);
...
static void remmina_plugin_spice_main_channel_event_cb(SpiceChannel * channel, SpiceChannelEvent event,
						       RemminaProtocolWidget * gp)
{
	TRACE_CALL(__func__);
	...
	case SPICE_CHANNEL_CLOSED:
		remmina_plugin_service->get_server_port(remmina_plugin_service->file_get_string(remminafile, "server"),
							XSPICE_DEFAULT_PORT, &server, &port);
...
static void remmina_plugin_spice_display_ready_cb(GObject * display, GParamSpec * param_spec,
						  RemminaProtocolWidget * gp)
{
	TRACE_CALL(__func__);
...
static void remmina_plugin_spice_call_feature(RemminaProtocolWidget * gp, const RemminaProtocolFeature * feature)
{
	...
	case REMMINA_PLUGIN_SPICE_FEATURE_PREF_VIEWONLY:
		g_object_set(gpdata->session,
			     "read-only", remmina_plugin_service->file_get_int(remminafile, "viewonly", FALSE), NULL);
		break;
	case REMMINA_PLUGIN_SPICE_FEATURE_PREF_RESIZEGUEST:
		g_object_set(gpdata->display,
			     "resize-guest",
			     remmina_plugin_service->file_get_int(remminafile, "resizeguest", TRUE), NULL);
		break;
...
static const RemminaProtocolSetting remmina_plugin_spice_basic_settings[] = {
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "resizeguest", N_("Resize guest to match window size"), FALSE, NULL,
	 NULL},
...
static const RemminaProtocolFeature remmina_plugin_spice_features[] = {
	{REMMINA_PROTOCOL_FEATURE_TYPE_PREF, REMMINA_PLUGIN_SPICE_FEATURE_PREF_VIEWONLY,
	 GINT_TO_POINTER(REMMINA_PROTOCOL_FEATURE_PREF_CHECK), "viewonly", N_("View only")},
	{REMMINA_PROTOCOL_FEATURE_TYPE_PREF, REMMINA_PLUGIN_SPICE_FEATURE_PREF_RESIZEGUEST,
	 GINT_TO_POINTER(REMMINA_PROTOCOL_FEATURE_PREF_CHECK), "resizeguest", N_("Resize guest to match window size")},
	{REMMINA_PROTOCOL_FEATURE_TYPE_PREF, REMMINA_PLUGIN_SPICE_FEATURE_PREF_DISABLECLIPBOARD,
	 GINT_TO_POINTER(REMMINA_PROTOCOL_FEATURE_PREF_CHECK), "disableclipboard", N_("Disable clipboard sync")},
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PLUGIN_SPICE_FEATURE_TOOL_SENDCTRLALTDEL,
	 N_("Send Ctrl+Alt+Delete"), NULL, NULL},
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PLUGIN_SPICE_FEATURE_TOOL_USBREDIR,
	 N_("Select USB devices for redirection"), NULL, NULL},
