...
void remmina_plugin_spice_select_usb_devices(RemminaProtocolWidget * gp)
{
	...
	dialog = gtk_dialog_new_with_buttons(_("Select USB devices for redirection"),
					     NULL, GTK_DIALOG_MODAL, _("_Close"), GTK_RESPONSE_ACCEPT, NULL);
	gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT);

	usb_device_widget = spice_usb_device_widget_new(gpdata->session, NULL);
	g_signal_connect(usb_device_widget,
			 "connect-failed", G_CALLBACK(remmina_plugin_spice_usb_connect_failed_cb), gp);

	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), usb_device_widget, TRUE, TRUE, 0);
	gtk_widget_show_all(dialog);
	gtk_dialog_run(GTK_DIALOG(dialog));
	gtk_widget_destroy(dialog);
}

static void remmina_plugin_spice_usb_connect_failed_cb(GObject * object, SpiceUsbDevice * usb_device, GError * error,
						       RemminaProtocolWidget * gp)
{
	...
	dialog = gtk_message_dialog_new(NULL,
					GTK_DIALOG_MODAL,
					GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, _("USB redirection error"));
