...
static void remmina_cell_renderer_pixbuf_class_init(RemminaCellRendererPixbufClass * klass)
{
	...
	remmina_cell_renderer_pixbuf_signals[0] = g_signal_new("activate", G_TYPE_FROM_CLASS(klass),
							       ...
							       G_STRUCT_OFFSET(RemminaCellRendererPixbufClass,
									       activate), NULL, NULL,
...
static void remmina_ftp_client_class_init(RemminaFTPClientClass * klass)
{
	...
	remmina_ftp_client_signals[NEW_TASK_SIGNAL] =
	    g_signal_new("new-task", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaFTPClientClass, new_task), NULL, NULL, g_cclosure_marshal_VOID__VOID,
			 G_TYPE_NONE, 0);
	remmina_ftp_client_signals[CANCEL_TASK_SIGNAL] =
	    g_signal_new("cancel-task", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaFTPClientClass, cancel_task), NULL, NULL, remmina_marshal_BOOLEAN__INT,
			 G_TYPE_BOOLEAN, 1, G_TYPE_INT);
	remmina_ftp_client_signals[DELETE_FILE_SIGNAL] =
	    g_signal_new("delete-file", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaFTPClientClass, delete_file), NULL, NULL,
...
static void remmina_ftp_client_cell_data_filetype_pixbuf(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
...
static void remmina_ftp_client_cell_data_progress_pixbuf(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
							 GtkTreeModel * model, GtkTreeIter * iter, gpointer user_data)
{
	TRACE_CALL("remmina_ftp_client_cell_data_progress_pixbuf");
	gint tasktype, status;

	gtk_tree_model_get(model, iter, REMMINA_FTP_TASK_COLUMN_TASKTYPE, &tasktype, REMMINA_FTP_TASK_COLUMN_STATUS,
			   &status, -1);
...
static void remmina_ftp_client_cell_data_size(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
					      GtkTreeModel * model, GtkTreeIter * iter, gpointer user_data)
{
...
static void remmina_ftp_client_cell_data_permission(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
						    GtkTreeModel * model, GtkTreeIter * iter, gpointer user_data)
{
...
static void remmina_ftp_client_cell_data_size_progress(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
						       GtkTreeModel * model, GtkTreeIter * iter, gpointer user_data)
{
...
static void remmina_ftp_client_cell_data_progress(GtkTreeViewColumn * col, GtkCellRenderer * renderer,
						  GtkTreeModel * model, GtkTreeIter * iter, gpointer user_data)
{
...
static void remmina_ftp_client_open_dir(RemminaFTPClient * client, const gchar * dir)
{
	...
	BUSY_CURSOR g_signal_emit(G_OBJECT(client), remmina_ftp_client_signals[OPEN_DIR_SIGNAL], 0, dir);
NORMAL_CURSOR}
...
static void remmina_ftp_client_download(RemminaFTPClient * client, GtkTreeIter * piter, const gchar * localdir)
{
	...
	gtk_tree_model_get(priv->file_list_sort, piter, REMMINA_FTP_FILE_COLUMN_TYPE, &type,
			   REMMINA_FTP_FILE_COLUMN_NAME, &name, REMMINA_FTP_FILE_COLUMN_SIZE, &size, -1);

	gtk_list_store_append(store, &iter);
	gtk_list_store_set(store, &iter, REMMINA_FTP_TASK_COLUMN_TYPE, type, REMMINA_FTP_TASK_COLUMN_NAME, name,
			   REMMINA_FTP_TASK_COLUMN_SIZE, size, REMMINA_FTP_TASK_COLUMN_TASKID,
			   remmina_ftp_client_taskid++, REMMINA_FTP_TASK_COLUMN_TASKTYPE,
			   REMMINA_FTP_TASK_TYPE_DOWNLOAD, REMMINA_FTP_TASK_COLUMN_REMOTEDIR, priv->current_directory,
			   REMMINA_FTP_TASK_COLUMN_LOCALDIR, localdir, REMMINA_FTP_TASK_COLUMN_STATUS,
			   REMMINA_FTP_TASK_STATUS_WAIT, REMMINA_FTP_TASK_COLUMN_DONESIZE, 0.0,
			   REMMINA_FTP_TASK_COLUMN_TOOLTIP, NULL, -1);
...
static gboolean remmina_ftp_client_task_list_on_query_tooltip(GtkWidget * widget, gint x, gint y,
...
static void remmina_ftp_client_action_delete(GObject * object, RemminaFTPClient * client)
{
	...
	BUSY_CURSOR list_iter = g_list_first(list);
	while (list_iter) {
		gtk_tree_model_get_iter(priv->file_list_sort, &iter, (GtkTreePath *) list_iter->data);
...
static void remmina_ftp_client_action_upload(GObject * object, RemminaFTPClient * client)
{
	...
	dialog = gtk_file_chooser_dialog_new(_("Choose a file to upload"),
					     GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(client))),
					     GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel", GTK_RESPONSE_CANCEL, "_OK",
	...
	for (element = files; element; element = element->next) {
		...
		gtk_list_store_append(store, &iter);
		gtk_list_store_set(store, &iter, REMMINA_FTP_TASK_COLUMN_TYPE, type, REMMINA_FTP_TASK_COLUMN_NAME,
				   name, REMMINA_FTP_TASK_COLUMN_SIZE, (gfloat) st.st_size,
				   ...
				   REMMINA_FTP_TASK_COLUMN_LOCALDIR, dir, REMMINA_FTP_TASK_COLUMN_STATUS,
				   REMMINA_FTP_TASK_STATUS_WAIT, REMMINA_FTP_TASK_COLUMN_DONESIZE, 0.0,
				   REMMINA_FTP_TASK_COLUMN_TOOLTIP, NULL, -1);
...
static gboolean remmina_ftp_client_file_list_on_button_press(GtkWidget * widget, GdkEventButton * event,
							     RemminaFTPClient * client)
{
	...
	if (event->button == 3) {
		...
		list =
		    gtk_tree_selection_get_selected_rows(gtk_tree_view_get_selection
							 (GTK_TREE_VIEW(priv->file_list_view)), NULL);
		if (list) {
...
static void remmina_ftp_client_task_list_cell_on_activate(GtkCellRenderer * renderer, gchar * path,
							  RemminaFTPClient * client)
{
...
static gboolean remmina_ftp_client_filter_visible_func(GtkTreeModel * model, GtkTreeIter * iter,
						       RemminaFTPClient * client)
{
...
static void remmina_ftp_client_init(RemminaFTPClient * client)
{
	...
	gtk_tree_view_column_set_cell_data_func(column, renderer, remmina_ftp_client_cell_data_filetype_pixbuf, NULL,
						NULL);
	...
	renderer = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Group"), renderer, "text", REMMINA_FTP_FILE_COLUMN_GROUP,
						     NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	...
	renderer = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Permission"), renderer, "text",
						     REMMINA_FTP_FILE_COLUMN_PERMISSION, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	...
	priv->file_list_model =
	    GTK_TREE_MODEL(gtk_list_store_new
			   (REMMINA_FTP_FILE_N_COLUMNS, G_TYPE_INT, G_TYPE_STRING, G_TYPE_FLOAT, G_TYPE_STRING,
	...
	renderer = gtk_cell_renderer_pixbuf_new();
	gtk_tree_view_column_pack_start(column, renderer, FALSE);
	gtk_tree_view_column_set_cell_data_func(column, renderer, remmina_ftp_client_cell_data_progress_pixbuf, NULL,
						NULL);
	renderer = gtk_cell_renderer_pixbuf_new();
	gtk_tree_view_column_pack_start(column, renderer, FALSE);
	gtk_tree_view_column_set_cell_data_func(column, renderer, remmina_ftp_client_cell_data_filetype_pixbuf, NULL,
	...
	renderer = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Remote"), renderer, "text", REMMINA_FTP_TASK_COLUMN_REMOTEDIR,
						     NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	...
	renderer = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Local"), renderer, "text", REMMINA_FTP_TASK_COLUMN_LOCALDIR,
						     NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	...
	gtk_tree_view_column_set_alignment(column, 1.0);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_cell_data_func(column, renderer, remmina_ftp_client_cell_data_size_progress, NULL,
						NULL);
	...
	g_signal_connect(G_OBJECT(renderer), "activate", G_CALLBACK(remmina_ftp_client_task_list_cell_on_activate),
			 client);

	/* Task List - Model */
	priv->task_list_model =
	    GTK_TREE_MODEL(gtk_list_store_new
			   (REMMINA_FTP_TASK_N_COLUMNS, G_TYPE_INT, G_TYPE_STRING, G_TYPE_FLOAT, G_TYPE_INT,
	...
	g_signal_connect(G_OBJECT(priv->directory_combo), "changed", G_CALLBACK(remmina_ftp_client_dir_on_changed),
			 client);
...
void remmina_ftp_client_add_file(RemminaFTPClient * client, ...)
{
	...
	gtk_tree_model_get(GTK_TREE_MODEL(store), &iter,
			   REMMINA_FTP_FILE_COLUMN_TYPE, &type, REMMINA_FTP_FILE_COLUMN_NAME, &name, -1);
...
RemminaFTPTask *remmina_ftp_client_get_waiting_task(RemminaFTPClient * client)
{
	...
	while (1) {
		gtk_tree_model_get(priv->task_list_model, &iter, REMMINA_FTP_TASK_COLUMN_TYPE, &task.type,
				   ...
				   REMMINA_FTP_TASK_COLUMN_TASKID, &task.taskid, REMMINA_FTP_TASK_COLUMN_TASKTYPE,
				   &task.tasktype, REMMINA_FTP_TASK_COLUMN_REMOTEDIR, &task.remotedir,
				   REMMINA_FTP_TASK_COLUMN_LOCALDIR, &task.localdir, REMMINA_FTP_TASK_COLUMN_STATUS,
				   &task.status, REMMINA_FTP_TASK_COLUMN_DONESIZE, &task.donesize,
				   REMMINA_FTP_TASK_COLUMN_TOOLTIP, &task.tooltip, -1);
...
void remmina_ftp_client_update_task(RemminaFTPClient * client, RemminaFTPTask * task)
{
	...
	gtk_tree_path_free(path);
	gtk_list_store_set(store, &iter, REMMINA_FTP_TASK_COLUMN_SIZE, task->size, REMMINA_FTP_TASK_COLUMN_STATUS,
			   task->status, REMMINA_FTP_TASK_COLUMN_DONESIZE, task->donesize,
			   REMMINA_FTP_TASK_COLUMN_TOOLTIP, task->tooltip, -1);
}
