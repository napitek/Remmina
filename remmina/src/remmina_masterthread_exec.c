...
static gboolean remmina_masterthread_exec_callback(RemminaMTExecData * d)
{
	...
	if (!d->cancelled) {
		...
		case FUNC_DIALOG_SERVERKEY_CONFIRM:
			d->p.dialog_serverkey_confirm.retval =
			    remmina_init_dialog_serverkey_confirm(d->p.dialog_serverkey_confirm.dialog,
								  d->p.dialog_serverkey_confirm.serverkey,
								  d->p.dialog_serverkey_confirm.prompt);
			break;
		case FUNC_DIALOG_AUTHPWD:
			d->p.dialog_authpwd.retval = remmina_init_dialog_authpwd(d->p.dialog_authpwd.dialog,
										 d->p.dialog_authpwd.label,
										 d->p.dialog_authpwd.allow_save);
			break;
		...
		case FUNC_DIALOG_AUTHUSERPWD:
			d->p.dialog_authuserpwd.retval =
			    remmina_init_dialog_authuserpwd(d->p.dialog_authuserpwd.dialog,
							    d->p.dialog_authuserpwd.want_domain,
							    d->p.dialog_authuserpwd.default_username,
							    d->p.dialog_authuserpwd.default_domain,
							    d->p.dialog_authuserpwd.allow_save);
			break;
		case FUNC_DIALOG_CERT:
			d->p.dialog_certificate.retval =
			    remmina_init_dialog_certificate(d->p.dialog_certificate.dialog,
							    d->p.dialog_certificate.subject,
							    d->p.dialog_certificate.issuer,
							    d->p.dialog_certificate.fingerprint);
			break;
		case FUNC_DIALOG_CERTCHANGED:
			d->p.dialog_certchanged.retval =
			    remmina_init_dialog_certificate_changed(d->p.dialog_certchanged.dialog,
								    d->p.dialog_certchanged.subject,
								    d->p.dialog_certchanged.issuer,
								    d->p.dialog_certchanged.new_fingerprint,
								    d->p.dialog_certchanged.old_fingerprint);
			break;
		case FUNC_DIALOG_AUTHX509:
			d->p.dialog_authx509.retval =
			    remmina_init_dialog_authx509(d->p.dialog_authx509.dialog, d->p.dialog_authx509.cacert,
							 d->p.dialog_authx509.cacrl, d->p.dialog_authx509.clientcert,
							 d->p.dialog_authx509.clientkey);
			break;
		case FUNC_FTP_CLIENT_UPDATE_TASK:
			remmina_ftp_client_update_task(d->p.ftp_client_update_task.client,
						       d->p.ftp_client_update_task.task);
			break;
...
#ifdef HAVE_LIBSSH
			d->p.sftp_client_confirm_resume.retval =
			    remmina_sftp_client_confirm_resume(d->p.sftp_client_confirm_resume.client,
							       d->p.sftp_client_confirm_resume.path);
#endif
			break;
		case FUNC_VTE_TERMINAL_SET_ENCODING_AND_PTY:
#if defined (HAVE_LIBSSH) && defined (HAVE_LIBVTE)
			remmina_plugin_ssh_vte_terminal_set_encoding_and_pty(d->p.vte_terminal_set_encoding_and_pty.
									     terminal,
									     d->p.vte_terminal_set_encoding_and_pty.
									     codeset,
									     d->p.vte_terminal_set_encoding_and_pty.
									     master,
									     d->p.vte_terminal_set_encoding_and_pty.
