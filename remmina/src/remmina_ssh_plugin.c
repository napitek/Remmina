...
static gpointer remmina_plugin_ssh_main_thread(gpointer data)
{
	...
	CANCEL_ASYNC gpdata = GET_PLUGIN_DATA(gp);
	...
	remmina_plugin_ssh_vte_terminal_set_encoding_and_pty(VTE_TERMINAL(gpdata->vte), charset, shell->master,
							     shell->slave);
...
void remmina_plugin_ssh_vte_terminal_set_encoding_and_pty(VteTerminal * terminal, const char *codeset, int master,
							  int slave)
{
...
gboolean remmina_ssh_plugin_popup_menu(GtkWidget * widget, GdkEvent * event, GtkWidget * menu)
{
	...
	if ((event->type == GDK_BUTTON_PRESS) && (((GdkEventButton *) event)->button == 3)) {
		...
		gtk_menu_popup(GTK_MENU(menu),
			       NULL,
			       NULL, NULL, NULL, ((GdkEventButton *) event)->button, gtk_get_current_event_time());
		return TRUE;
	}
...
void remmina_plugin_ssh_popup_ui(RemminaProtocolWidget * gp)
{
	...
	g_signal_connect(G_OBJECT(gpdata->vte), "button_press_event", G_CALLBACK(remmina_ssh_plugin_popup_menu), menu);

	g_signal_connect(G_OBJECT(select_all), "activate", G_CALLBACK(remmina_plugin_ssh_vte_select_all), gpdata->vte);
	g_signal_connect(G_OBJECT(copy), "activate", G_CALLBACK(remmina_plugin_ssh_vte_copy_clipboard), gpdata->vte);
	g_signal_connect(G_OBJECT(paste), "activate", G_CALLBACK(remmina_plugin_ssh_vte_paste_clipboard), gpdata->vte);
...
static void remmina_plugin_ssh_init(RemminaProtocolWidget * gp)
{
	...
	if (remmina_pref.vte_system_colors) {
		...
		gtk_style_context_get(style_context, GTK_STATE_FLAG_NORMAL, "background-color", &background_color,
				      NULL);
	} else {
	...
	vte_terminal_set_colors(VTE_TERMINAL(vte), &foreground_color, &background_color, remminavte.palette,
				PALETTE_SIZE);
...
static void remmina_plugin_ssh_call_feature(RemminaProtocolWidget * gp, const RemminaProtocolFeature * feature)
{
	...
	case REMMINA_PROTOCOL_FEATURE_TOOL_SSH:
		remmina_plugin_service->
		    open_connection(remmina_file_dup_temp_protocol
				    (remmina_plugin_service->protocol_plugin_get_file(gp), "SSH"), NULL, gpdata->shell,
				    NULL);
		return;
	case REMMINA_PROTOCOL_FEATURE_TOOL_SFTP:
		remmina_plugin_service->
		    open_connection(remmina_file_dup_temp_protocol
				    (remmina_plugin_service->protocol_plugin_get_file(gp), "SFTP"), NULL,
				    gpdata->shell, NULL);
		return;
...
static RemminaProtocolFeature remmina_plugin_ssh_features[] = {
	...
	{REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PLUGIN_SSH_FEATURE_TOOL_SELECT_ALL, N_("Select all"),
	 N_("_Select all"), NULL},
...
static const RemminaProtocolSetting remmina_ssh_advanced_settings[] = {
	...
	{REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "ssh_color_scheme", N_("Terminal color scheme"), FALSE,
	 ssh_terminal_palette, NULL},
