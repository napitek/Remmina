...
static void remmina_applet_menu_class_init(RemminaAppletMenuClass * klass)
{
	...
	remmina_applet_menu_signals[LAUNCH_ITEM_SIGNAL] = g_signal_new("launch-item", G_TYPE_FROM_CLASS(klass),
								       ...
								       G_STRUCT_OFFSET(RemminaAppletMenuClass,
										       launch_item), NULL, NULL,
								       g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
								       G_TYPE_OBJECT);
	remmina_applet_menu_signals[EDIT_ITEM_SIGNAL] =
	    g_signal_new("edit-item", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaAppletMenuClass, edit_item), NULL, NULL,
...
static GtkWidget *remmina_applet_menu_add_group(GtkWidget * menu, const gchar * group, gint position,
						RemminaAppletMenuItem * menuitem, GtkWidget ** groupmenuitem)
{
	...
	image =
	    gtk_image_new_from_icon_name((menuitem->item_type == REMMINA_APPLET_MENU_ITEM_DISCOVERED ?
			...
			if (gtk_menu_item_get_submenu(submenuitem)) {
				...
				if (g_strcmp0(p1, mstr) == 0) {
					...
					if (strcoll(p1, mstr) < 0) {
						submenu =
						    remmina_applet_menu_add_group(submenu, p1, position, menuitem,
										  &groupmenuitem);
						break;
					}
				}
				...
				submenu =
				    remmina_applet_menu_add_group(submenu, p1, position, menuitem, &groupmenuitem);
				break;
			}
