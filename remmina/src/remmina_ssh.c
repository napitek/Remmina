...
static gint remmina_ssh_auth_password(RemminaSSH * ssh)
{
...
static gint remmina_ssh_auth_pubkey(RemminaSSH * ssh)
{
...
static gint remmina_ssh_auth_auto_pubkey(RemminaSSH * ssh)
{
...
static gint remmina_ssh_auth_agent(RemminaSSH * ssh)
{
	...
	case REMMINA_SSH_TUNNEL_OPEN:
		...
		if (ssh_channel_open_forward(channel, tunnel->dest, tunnel->port, "127.0.0.1", 0) != SSH_OK) {
			...
			remmina_ssh_set_error(REMMINA_SSH(tunnel),
					      _("Failed to connect to the SSH tunnel destination: %s"));
			tunnel->thread = 0;
			return NULL;
		}
	...
	case REMMINA_SSH_TUNNEL_XPORT:
		...
		for (i = 10; i <= MAX_X_DISPLAY_NUMBER; i++) {
			G_GNUC_BEGIN_IGNORE_DEPRECATIONS
				...
				(REMMINA_SSH(tunnel)->session, (tunnel->bindlocalhost ? "localhost" : NULL), 6000 + i,
				 NULL)) {
				continue;
			} else {
		...
		if (tunnel->tunnel_type == REMMINA_SSH_TUNNEL_XPORT ||
			...
			if (first) {
				...
				if (tunnel->tunnel_type == REMMINA_SSH_TUNNEL_X11) {
					...
					channel =
					    ssh_channel_accept_forward(REMMINA_SSH(tunnel)->session, 15000,
								       &tunnel->port);
				}
				if (!channel) {
				...
				if (diff > 1) {
					if (tunnel->tunnel_type == REMMINA_SSH_TUNNEL_X11) {
						...
						channel =
						    ssh_channel_accept_forward(REMMINA_SSH(tunnel)->session, 0,
									       &tunnel->port);
					}
					if (channel == NULL) {
						t2 = t1;
		...
		while (tunnel->running && i < tunnel->num_channels) {
			...
			if (!tunnel->socketbuffers[i]) {
				...
				if (len == SSH_ERROR || len == SSH_EOF) {
					...
					len =
					    ssh_channel_read_nonblocking(tunnel->channels[i],
									 tunnel->socketbuffers[i]->data, len, 0);
					if (len <= 0) {
						remmina_ssh_set_error(REMMINA_SSH(tunnel),
			...
			if (!disconnected && tunnel->socketbuffers[i]) {
				for (lenw = 0; tunnel->socketbuffers[i]->len > 0;
					...
					lenw =
					    write(tunnel->sockets[i], tunnel->socketbuffers[i]->ptr,
						  tunnel->socketbuffers[i]->len);
