					 ...
					 const gchar * remote_path, const gchar * local_path, guint64 * donesize)
{
	...
	if (size > 0) {
		...
		case GTK_RESPONSE_ACCEPT:
			...
			if (!local_file) {
				remmina_sftp_client_thread_set_error(client, task, _("Error creating file %s."),
								     local_path);
				return FALSE;
			}
			size = 0;
	...
	while ((sftpattr = sftp_readdir(sftp->sftp_sess, sftpdir))) {
		if (g_strcmp0(sftpattr->name, ".") != 0 && g_strcmp0(sftpattr->name, "..") != 0) {
			...
			if (type == REMMINA_FTP_FILE_TYPE_DIR) {
				ret =
				    remmina_sftp_client_thread_recursive_dir(client, sftp, task, rootdir_path,
									     file_path, array);
				g_free(file_path);
				if (!ret) {
					      ...
					      const gchar * rootdir_path, const gchar * subdir_path, GPtrArray * array)
{
	...
	while ((name = g_dir_read_name(dir)) != NULL) {
		...
		if (g_file_test(abspath, G_FILE_TEST_IS_DIR)) {
			ret =
			    remmina_sftp_client_thread_recursive_localdir(client, task, rootdir_path, relpath, array);
			if (!ret)
				break;
		} else {
...
static gboolean
remmina_sftp_client_thread_mkdir(RemminaSFTPClient * client, RemminaSFTP * sftp, RemminaFTPTask * task,
				 const gchar * path)
{
				       ...
				       const gchar * remote_path, const gchar * local_path, guint64 * donesize)
{
	...
	if (size > 0) {
		...
		case GTK_RESPONSE_ACCEPT:
			...
			g_free(tmp);
			if (!remote_file) {
				remmina_sftp_client_thread_set_error(client, task,
								     _("Error creating file %s on server. %s"),
								     remote_path,
								     ssh_get_error(REMMINA_SSH(client->sftp)->
										   session));
				return FALSE;
			}
			size = 0;
	...
	while (task) {
		...
		if (!sftp) {
			...
			if (!remmina_ssh_init_session(REMMINA_SSH(sftp)) ||
			    remmina_ssh_auth(REMMINA_SSH(sftp), NULL) <= 0 || !remmina_sftp_open(sftp)) {
		...
		case REMMINA_FTP_TASK_TYPE_DOWNLOAD:
			...
			case REMMINA_FTP_FILE_TYPE_DIR:
				...
				ret =
				    remmina_sftp_client_thread_recursive_dir(client, sftp, task, remote, NULL, array);
				if (ret) {
					for (i = 0; i < array->len; i++) {
						...
						remote_file =
						    remmina_public_combine_path(remote,
										(gchar *) g_ptr_array_index(array, i));
						local_file =
						    remmina_public_combine_path(local,
										(gchar *) g_ptr_array_index(array, i));
						ret =
						    remmina_sftp_client_thread_download_file(client, sftp, task,
					...
					for (i = 0; i < array->len; i++) {
						...
						remote_file =
						    remmina_public_combine_path(remote,
										(gchar *) g_ptr_array_index(array, i));
						local_file =
						    g_build_filename(local, (gchar *) g_ptr_array_index(array, i),
								     NULL);
						if (g_file_test(local_file, G_FILE_TEST_IS_DIR)) {
							ret =
							    remmina_sftp_client_thread_mkdir(client, sftp, task,
											     remote_file);
						} else {
							ret =
							    remmina_sftp_client_thread_upload_file(client, sftp, task,
												   remote_file,
												   local_file, &size);
						}
						g_free(remote_file);
	...
	while ((sftpattr = sftp_readdir(client->sftp->sftp_sess, sftpdir))) {
		if (g_strcmp0(sftpattr->name, ".") != 0 && g_strcmp0(sftpattr->name, "..") != 0) {
			...
			remmina_ftp_client_add_file(REMMINA_FTP_CLIENT(client),
						    ...
						    REMMINA_FTP_FILE_COLUMN_PERMISSION, sftpattr->permissions, -1);
			g_free(tmp);
		}
		sftp_attributes_free(sftpattr);
	}
