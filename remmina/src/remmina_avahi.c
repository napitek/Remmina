...
static void
remmina_avahi_resolve_callback(AvahiServiceResolver * r,
...
static void
remmina_avahi_browse_callback(AvahiServiceBrowser * b,
...
void remmina_avahi_start(RemminaAvahi * ga)
{
	...
	ga->priv->client =
	    avahi_client_new(avahi_simple_poll_get(ga->priv->simple_poll), 0, remmina_avahi_client_callback, ga,
			     &error);
	if (!ga->priv->client) {
	...
	ga->priv->sb =
	    avahi_service_browser_new(ga->priv->client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, "_rfb._tcp", NULL, 0,
