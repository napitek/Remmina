...
static void remmina_protocol_widget_class_init(RemminaProtocolWidgetClass * klass)
{
	...
	remmina_protocol_widget_signals[CONNECT_SIGNAL] = g_signal_new("connect", G_TYPE_FROM_CLASS(klass),
								       ...
								       G_STRUCT_OFFSET(RemminaProtocolWidgetClass,
										       connect), NULL, NULL,
								       g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	remmina_protocol_widget_signals[DISCONNECT_SIGNAL] =
	    g_signal_new("disconnect", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaProtocolWidgetClass, disconnect), NULL, NULL,
			 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	remmina_protocol_widget_signals[DESKTOP_RESIZE_SIGNAL] =
	    g_signal_new("desktop-resize", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaProtocolWidgetClass, desktop_resize), NULL, NULL,
			 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
	remmina_protocol_widget_signals[UPDATE_ALIGN_SIGNAL] =
	    g_signal_new("update-align", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			 G_STRUCT_OFFSET(RemminaProtocolWidgetClass, update_align), NULL, NULL,
			 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void remmina_protocol_widget_init_cancel(RemminaInitDialog * dialog, gint response_id,
						RemminaProtocolWidget * gp)
{
...
static void remmina_protocol_widget_show_init_dialog(RemminaProtocolWidget * gp, const gchar * name)
{
	...
	g_signal_connect(G_OBJECT(gp->priv->init_dialog), "response", G_CALLBACK(remmina_protocol_widget_init_cancel),
			 gp);
...
void remmina_protocol_widget_call_feature_by_ref(RemminaProtocolWidget * gp, const RemminaProtocolFeature * feature)
{
	...
	case REMMINA_PROTOCOL_FEATURE_TOOL_SSH:
		if (gp->priv->ssh_tunnel) {
			remmina_connection_window_open_from_file_full(remmina_file_dup_temp_protocol
								      (gp->priv->remmina_file, "SSH"), NULL,
								      gp->priv->ssh_tunnel, NULL);
			return;
		}
		break;
	...
	case REMMINA_PROTOCOL_FEATURE_TOOL_SFTP:
		if (gp->priv->ssh_tunnel) {
			remmina_connection_window_open_from_file_full(remmina_file_dup_temp_protocol
								      (gp->priv->remmina_file, "SFTP"), NULL,
								      gp->priv->ssh_tunnel, NULL);
			return;
		}
		break;
...
static gboolean remmina_protocol_widget_on_key_press(GtkWidget * widget, GdkEventKey * event,
						     RemminaProtocolWidget * gp)
{
...
static gboolean remmina_protocol_widget_on_key_release(GtkWidget * widget, GdkEventKey * event,
						       RemminaProtocolWidget * gp)
{
...
void remmina_protocol_widget_register_hostkey(RemminaProtocolWidget * gp, GtkWidget * widget)
{
	...
	g_signal_connect(G_OBJECT(widget), "key-release-event", G_CALLBACK(remmina_protocol_widget_on_key_release),
			 gp);
}
...
static gboolean remmina_protocol_widget_init_tunnel(RemminaProtocolWidget * gp)
{
	...
	if (gp->priv->ssh_tunnel == NULL) {
		...
		ret =
		    remmina_ssh_auth_gui(REMMINA_SSH(tunnel), REMMINA_INIT_DIALOG(gp->priv->init_dialog),
					 gp->priv->remmina_file);
		if (ret <= 0) {
			if (ret == 0) {
				remmina_protocol_widget_set_error(gp, REMMINA_SSH(tunnel)->error);
			}
...
gboolean remmina_protocol_widget_start_reverse_tunnel(RemminaProtocolWidget * gp, gint local_port)
{
	...
	remmina_init_dialog_set_status(REMMINA_INIT_DIALOG(gp->priv->init_dialog),
				       _("Waiting for an incoming SSH tunnel at port %i..."),
				       remmina_file_get_int(gp->priv->remmina_file, "listenport", 0));

	if (!remmina_ssh_tunnel_reverse
	    (gp->priv->ssh_tunnel, remmina_file_get_int(gp->priv->remmina_file, "listenport", 0), local_port)) {
		remmina_protocol_widget_set_error(gp, REMMINA_SSH(gp->priv->ssh_tunnel)->error);
		return FALSE;
	}
#endif
...
gboolean remmina_protocol_widget_ssh_exec(RemminaProtocolWidget * gp, gboolean wait, const gchar * fmt, ...)
{
	...
	if (ssh_channel_open_session(channel) == SSH_OK && ssh_channel_request_exec(channel, cmd) == SSH_OK) {
		if (wait) {
			...
			default:
				remmina_ssh_set_application_error(REMMINA_SSH(tunnel),
								  _("Command %s failed on SSH server (status = %i)."),
								  cmd, status);
				break;
			}
		} else {
...
static gboolean remmina_protocol_widget_tunnel_init_callback(RemminaSSHTunnel * tunnel, gpointer data)
{
	...
	ret = ((RemminaXPortTunnelInitFunc) gp->priv->init_func) (gp,
								  ...
								  (tunnel->bindlocalhost ? "localhost" : server),
								  port);
	g_free(server);
...
gboolean remmina_protocol_widget_start_xport_tunnel(RemminaProtocolWidget * gp, RemminaXPortTunnelInitFunc init_func)
{
	...
	remmina_init_dialog_set_status(REMMINA_INIT_DIALOG(gp->priv->init_dialog),
				       _("Connecting to %s through SSH tunnel..."),
...
#ifdef HAVE_LIBSSH
	if (gp->priv->ssh_tunnel->localdisplay)
		g_free(gp->priv->ssh_tunnel->localdisplay);
					  ...
					  gboolean allow_password_saving)
{
	...
	ret = remmina_init_dialog_authpwd(REMMINA_INIT_DIALOG(gp->priv->init_dialog),
					  s, remmina_file_get_filename(remminafile) != NULL && allow_password_saving);
	g_free(s);
...
gint remmina_protocol_widget_init_certificate(RemminaProtocolWidget * gp, const gchar * subject, const gchar * issuer,
					      const gchar * fingerprint)
{
	TRACE_CALL("remmina_protocol_widget_init_certificate");
	return remmina_init_dialog_certificate(REMMINA_INIT_DIALOG(gp->priv->init_dialog), subject, issuer,
					       fingerprint);
}

gint remmina_protocol_widget_changed_certificate(RemminaProtocolWidget * gp, const gchar * subject,
						 const gchar * issuer, const gchar * new_fingerprint,
						 const gchar * old_fingerprint)
{
	TRACE_CALL("remmina_protocol_widget_changed_certificate");
	return remmina_init_dialog_certificate_changed(REMMINA_INIT_DIALOG(gp->priv->init_dialog), subject, issuer,
...
void remmina_protocol_widget_send_keys_signals(GtkWidget * widget, const guint * keyvals, int keyvals_length,
					       GdkEventType action)
{
	...
	if (action & GDK_KEY_PRESS) {
		...
		for (i = 0; i < keyvals_length; i++) {
			...
			event.is_modifier =
			    (int) remmina_public_get_modifier_for_keycode(keymap, event.hardware_keycode);
