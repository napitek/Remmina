...
static const gchar *charset_list = "ASCII,BIG5,"
    ...
    "ISO-8859-13,ISO-8859-14,ISO-8859-15,ISO-8859-16," "KOI8-R,SJIS,UTF-8";
#endif
...
static GtkWidget *remmina_file_editor_create_notebook_tab(RemminaFileEditor * gfe,
							  const gchar * stock_id, const gchar * label, gint rows,
							  gint cols)
{
...
static void remmina_file_editor_ssh_server_custom_radio_on_toggled(GtkToggleButton * togglebutton,
								   RemminaFileEditor * gfe)
{
...
static void remmina_file_editor_ssh_auth_publickey_radio_on_toggled(GtkToggleButton * togglebutton,
								    RemminaFileEditor * gfe)
{
...
static void remmina_file_editor_create_ssh_privatekey(RemminaFileEditor * gfe, GtkWidget * grid, gint row, gint column)
{
	...
	widget =
	    gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(priv->ssh_auth_password_radio),
							_("Identity file"));
	...
	if (ssh_privatekey && g_file_test(ssh_privatekey, G_FILE_TEST_IS_REGULAR | G_FILE_TEST_EXISTS)) {
		gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(priv->ssh_privatekey_chooser), ssh_privatekey);
	} else {
...
static void remmina_file_editor_create_server(RemminaFileEditor * gfe, const RemminaProtocolSetting * setting,
					      GtkWidget * grid, gint row)
{
	...
	widget =
	    remmina_public_create_combo_entry(s, remmina_file_get_string(gfe->priv->remmina_file, "server"), TRUE);
						  ...
						  GtkWidget * grid, gint row)
{
	...
	widget =
	    gtk_radio_button_new_with_label(NULL, setting->opt1 ? _("Use window size") : _("Use client resolution"));
	gtk_widget_show(widget);
	gtk_grid_attach(GTK_GRID(grid), widget, 1, row, 1, 1);
	...
	widget =
	    gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(gfe->priv->resolution_auto_radio),
							_("Custom"));
	gtk_widget_show(widget);
	gtk_box_pack_start(GTK_BOX(hbox), widget, FALSE, FALSE, 0);
...
static GtkWidget *remmina_file_editor_create_chooser(RemminaFileEditor * gfe, GtkWidget * grid, gint row, gint col,
	...
	while (settings->type != REMMINA_PROTOCOL_SETTING_TYPE_END) {
		...
		case REMMINA_PROTOCOL_SETTING_TYPE_PASSWORD:
			widget = remmina_file_editor_create_password(gfe, grid, grid_row, 0,
								     ...
								     remmina_file_get_string(priv->remmina_file,
											     setting_name));
		...
		case REMMINA_PROTOCOL_SETTING_TYPE_CHECK:
			widget = remmina_file_editor_create_check(gfe, grid, grid_row, grid_column,
								  g_dgettext(priv->plugin->domain, settings->label),
								  remmina_file_get_int(priv->remmina_file,
										       setting_name, FALSE));
			g_hash_table_insert(priv->setting_widgets, setting_name, widget);
			break;
...
static void remmina_file_editor_create_ssh_tab(RemminaFileEditor * gfe, RemminaProtocolSSHSetting ssh_setting)
{
	...
	grid = remmina_file_editor_create_notebook_tab(gfe, "dialog-password", "SSH Tunnel", 9, 3);
	...
	case REMMINA_PROTOCOL_SSH_SETTING_TUNNEL:
		...
		widget =
		    gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(priv->ssh_server_default_radio),
								_("Custom"));
		gtk_widget_show(widget);
		gtk_grid_attach(GTK_GRID(grid), widget, 0, row, 1, 1);
		g_signal_connect(G_OBJECT(widget), "toggled",
	...
	case REMMINA_PROTOCOL_SSH_SETTING_REVERSE_TUNNEL:
		...
		priv->ssh_server_entry = remmina_file_editor_create_text(gfe, grid, 1, 0, _("Server"), NULL);
	...
	if (!(g_strcmp0(p, "SFTP") == 0 || g_strcmp0(p, "SSH") == 0)) {
		priv->ssh_charset_combo = remmina_file_editor_create_combo(gfe, grid, row + 3, 0,
									   _("Character set"), charset_list,
									   remmina_file_get_string(priv->remmina_file,
												   "ssh_charset"));
		row++;
	}
	if (ssh_setting == REMMINA_PROTOCOL_SSH_SETTING_SFTP) {
		widget = remmina_file_editor_create_text(gfe, grid, row + 8, 1, _("Startup path"), NULL);
	...
	if (!(g_strcmp0(p, "SFTP") == 0 || g_strcmp0(p, "SSH") == 0)) {
		...
		if (ssh_setting == REMMINA_PROTOCOL_SSH_SETTING_TUNNEL ||
			...
			priv->ssh_username_entry =
			    remmina_file_editor_create_text(gfe, grid, row + 10, 0, _("User name"), NULL);
			row++;
		}
		...
		widget =
		    gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(priv->ssh_auth_agent_radio),
								_("Password"));
		gtk_widget_show(widget);
	...
	if (ssh_setting == REMMINA_PROTOCOL_SSH_SETTING_TUNNEL) {
		...
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(cs ?
							       priv->ssh_server_custom_radio : priv->
							       ssh_server_default_radio), TRUE);
		gtk_entry_set_text(GTK_ENTRY(priv->ssh_server_entry), cs ? cs : "");
	...
	if (!(g_strcmp0(p, "SFTP") == 0 || g_strcmp0(p, "SSH") == 0)) {
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON
					     (remmina_file_get_int(priv->remmina_file, "ssh_auth", 0) ==
					      SSH_AUTH_PUBLICKEY ? priv->
					      ssh_auth_publickey_radio : remmina_file_get_int(priv->remmina_file,
											      "ssh_auth",
											      0) ==
					      SSH_AUTH_AUTO_PUBLICKEY ? priv->
					      ssh_auth_auto_publickey_radio : remmina_file_get_int(priv->remmina_file,
												   "ssh_auth",
												   0) ==
					      SSH_AUTH_AGENT ? priv->ssh_auth_agent_radio : priv->
					      ssh_auth_password_radio), TRUE);
...
static void remmina_file_editor_update_ssh(RemminaFileEditor * gfe)
{
	...
	remmina_file_set_int(priv->remmina_file, "ssh_auth",
			     (priv->ssh_auth_publickey_radio
			      ...
			      SSH_AUTH_PUBLICKEY : priv->ssh_auth_auto_publickey_radio
			      && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->ssh_auth_auto_publickey_radio)) ?
			      SSH_AUTH_AUTO_PUBLICKEY : priv->ssh_auth_agent_radio
			      && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->ssh_auth_agent_radio)) ?
			      SSH_AUTH_AGENT : SSH_AUTH_PASSWORD));
	remmina_file_set_string(priv->remmina_file, "ssh_privatekey",
				(priv->
				 ssh_privatekey_chooser ?
				 gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(priv->ssh_privatekey_chooser)) :
				 NULL));
}
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		if (GTK_IS_ENTRY(value)) {
			...
			remmina_file_set_string(priv->remmina_file,
						...
						gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(value)) : NULL);
...
static void remmina_file_editor_update(RemminaFileEditor * gfe)
{
	...
	remmina_file_set_string(priv->remmina_file, "precommand",
				gtk_entry_get_text(GTK_ENTRY(priv->precommand_entry)));
	remmina_file_set_string(priv->remmina_file, "postcommand",
				gtk_entry_get_text(GTK_ENTRY(priv->postcommand_entry)));
	...
	if (priv->resolution_auto_radio) {
		remmina_file_set_string_ref(priv->remmina_file,
					    ...
					    (gtk_toggle_button_get_active
					     (GTK_TOGGLE_BUTTON(priv->resolution_auto_radio)) ? NULL :
					     remmina_public_combo_get_active_text(GTK_COMBO_BOX
										  (priv->resolution_custom_combo))));
	}
...
GtkWidget *remmina_file_editor_new_from_file(RemminaFile * remminafile)
{
	...
	remmina_plugin_manager_for_each_plugin(REMMINA_PLUGIN_TYPE_PROTOCOL, remmina_file_editor_iterate_protocol,
					       gfe);
